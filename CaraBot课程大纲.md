# CaraBot 企业级 RAG 系统课程大纲

## 🎯 课程目标
掌握企业级 RAG（检索增强生成）系统的设计、实现与运维，成为 AI 应用开发工程师。
...
1. **API 层** (`src/app/api/routes/`)
   ...
   - 包含健康检查、文档上传、搜索、统计等接口
   - 使用 FastAPI 的路由装饰器定义接口

2. **服务层** (`src/app/services/`)
   - 实现业务逻辑，类似 Java 的 Service
   - `UploadService`：处理文档上传的完整流程
   - `SearchService`：处理相似性搜索
   - `EmbeddingService`：处理文本向量嵌入

3. **向量存储层** (`src/app/vectorstore/`)
   - 抽象向量数据库操作，支持 Milvus 和 Chroma
   - `BaseVectorStore`：定义统一接口
   - `MilvusStore`：生产环境向量存储
   - `ChromaStore`：开发环境向量存储

4. **核心配置** (`src/app/core/config.py`)
...
## 📅 学习计划建议
- 每周 3-4 课，约 2-3 个月完成全部课程
- 每课配套实践作业，巩固所学知识
- 课程结束后完成一个完整的 RAG 项目实战
